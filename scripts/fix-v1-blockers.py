...
Path("src/terminal_io_windows.zig").write_text(
...
Path("src/rpc/transport_windows.zig").write_text(
    r'''const std = @import("std");
...
build = Path("build.zig")
build_text = build.read_text()
needle = '    const zlua = zlua_dep.module("zlua");\n'
if needle not in build_text:
    raise SystemExit("expected zlua module declaration not found")
if "zlua.optimize = zlua_optimize;" not in build_text:
    build.write_text(build_text.replace(needle, needle + "    zlua.optimize = zlua_optimize;\n", 1))
