...
function clientHoleRead() {
  ...
  throw new signals.NotReadyError(CLIENT_HOLE);
}
let Observer = null;
...
function createMemo(compute, options) {
  ...
  if (ssrSource === "client") {
    ...
    if (loadingState) {
      ...
      comp.error = new signals.NotReadyError(CLIENT_HOLE);
      comp.errored = true;
    }
  ...
  if (typeof result?.[Symbol.asyncIterator] !== "function" && isThenable(result)) {
    ...
    if (result.s === 3) {
      ...
      if (loadingState) {
        ...
        comp.error = new signals.NotReadyError(d.promise);
        comp.errored = true;
      }
      return;
    }
      ...
      if (effectFn && ctx?.async) {
        ...
        const retry = () => {
          ...
          try {
            ...
            if (retryErr instanceof signals.NotReadyError) {
...
function storeSetter(state) {
  return fn => {
    ...
    if (result !== undefined && result !== state && signals.isWrappable(result)) {
...
function createErrorBoundary(fn, fallback) {
  ...
  const resolve = () => {
    ...
    if (pending) {
      ...
      throw new signals.NotReadyError(all);
    }
    return resolved;
  };
...
exports.registerSlotPatch = registerSlotPatch;
exports.repeat = repeat;
